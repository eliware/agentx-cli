...
Goal mode exposes one `goal_update` function instead of separate lifecycle functions. Its required `method` is one of `complete`, `incomplete`, `blocked`, or `question`; optional fields carry summary/evidence or question/choices. Goal work requests require a tool call. After ordinary work-tool results, the next request restricts required tool selection to `goal_update`, preventing prose-only continuation loops. `complete` stops live status rendering, sends the completion acknowledgement, reports the final response usage and completion timing, and returns the final response to the interactive prompt, `blocked` ends it as blocked, `question` pauses for user input and resumes, and `incomplete` continues autonomous work.
