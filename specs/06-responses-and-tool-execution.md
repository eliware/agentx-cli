...
Usage from every response in the isolated image-inspection branch, including shell-continuation responses, is added to the parent session totals. Only the branch's text response is returned as the `view_image` tool output. Temporary converted data and branch checkpoint state are discarded after completion. The main conversation continues from its original response ID, so image bytes are not included in the main conversation chain.
