...
Recovery tests must verify that manual retry recreates the Responses client, that a failed tool-continuation retry cannot be silently replayed as the next new user request, and that abandoned pending retry state is cleared or requires an explicit resume action.
