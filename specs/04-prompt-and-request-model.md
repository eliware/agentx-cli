...
The prompt template may define the `view_image` function, but AgentX must expose/send it only for the normal interactive session and only when image inspection is enabled. Image-inspection branch requests must override the template tool list with an empty list and must not merge MCP tools.
