...
The prompt template may define the `view_image` function and the built-in `image_generation` tool. AgentX exposes/sends it to normal sessions and delegated one-shot workers when image inspection is enabled. Spawned workers still receive no orchestration tools (`spawn_agent`, `agent_status`, or `cancel_agent`). Image-inspection branch requests must override the template tool list with an empty list and must not merge MCP tools.
