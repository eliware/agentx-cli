...
MCP config is optional at `$HOME/.agentx.mcp.json`. Accept either a top-level array or `{ tools: [...] }`; merge entries after template tools. An MCP entry with `enabled: false` is retained in configuration but omitted from requests, allowing each server to be toggled without deleting credentials. Missing file is allowed; invalid JSON is fatal and should be reported as a prompt-template load error.
