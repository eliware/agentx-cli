...
The integration delegates Responses transport, connection lifecycle, streaming, event normalization, and API errors to `@eliware/openai`. AgentX owns session state, tool dispatch, user-facing recovery, and bounded recreation of the Responses client after recognized closed/lifetime failures. Tool calls execute sequentially by default; no parallel tool execution is allowed. Tool calls can cause multiple sequential Responses requests as outputs are returned to the model. Each assistant-turn tool call has one dispatch identity and must not be re-enqueued or executed more than once.
