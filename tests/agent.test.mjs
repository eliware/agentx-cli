...
describe('agent loop', () => {
  ...
  test('persists the active response id and pending tool calls while tool execution is in flight', async () => {
    ...
    await jest.unstable_mockModule('../src/response.mjs', () => ({ extractTextFromResponse: typeof extractTextFromResponse === 'undefined' ? jest.fn(() => '') : extractTextFromResponse, createUsageTotals: () => ({ inputTokens: 0, cachedTokens: 0, outputTokens: 0, turns: 0 }), addUsageTotals: (totals, usage) => { totals.inputTokens += usage.inputTokens || 0; totals.cachedTokens += usage.cachedTokens || 0; totals.outputTokens += usage.outputTokens || 0; return totals; }, formatUsageReport: (value) => JSON.stringify({ turns: String(value.turns ?? 0) }), formatTurnUsageReport: () => '', extractUsage: () => ({ inputTokens: 0, cachedTokens: 0, outputTokens: 0 }) }));
