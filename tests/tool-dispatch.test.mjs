...
describe('tool dispatch', () => {
  test('runs shell tool calls, handles parse errors and unsupported tools', async () => {
    ...
    try {
      ...
      expect(await runToolCall({ type: 'function_call', name: 'shell_call', input: { toString() { throw { boom: true }; } } }, '/opt/agentx-cli')).toContain('invalid shell_call input');
      ...
      expect(await runToolCall({ type: 'weird' }, '/opt/agentx-cli')).toBe('ERROR: unsupported tool weird');
