...
describe('tool dispatch', () => {
  ...
  test('runs shell tool calls and rejects unsupported tool calls', async () => {
    ...
    try {
      ...
      expect(await runToolCall({ type: 'function_call', name: 'agent_status', arguments: JSON.stringify({ agent_ids: ['missing-agent'] }) }, tmp)).toEqual({ agents: [{ id: 'missing-agent', status: 'unknown' }], waited_ms: 0, timed_out: false });
