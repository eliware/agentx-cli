...
describe('coverage gaps', () => {
  ...
  test('session helpers cover state persistence and fallback parsing', async () => {
    ...
    await persistResponseState(statePath, { response_id: 'resp-1', usage: { inputTokens: 1, cachedTokens: 2, outputTokens: 3, turns: 4 } });
    expect(await readSessionState(statePath)).toEqual({ response_id: 'resp-1', usage: { inputTokens: 1, cachedTokens: 2, outputTokens: 3, turns: 4 } });
    expect(await readSessionState(missingStatePath)).toBeNull();
    await writeText(statePath, 'resp-legacy\n');
    expect(await readSessionState(statePath)).toEqual({ response_id: 'resp-legacy', usage: { inputTokens: 0, cachedTokens: 0, outputTokens: 0, turns: 0 } });
    await writeText(statePath, '123');
    expect(await readSessionState(statePath)).toEqual({ response_id: '123', usage: { inputTokens: 0, cachedTokens: 0, outputTokens: 0, turns: 0 } });
    await writeText(statePath, 'not-json');
    expect(await readSessionState(statePath)).toEqual({ response_id: 'not-json', usage: { inputTokens: 0, cachedTokens: 0, outputTokens: 0, turns: 0 } });
    await writeText(statePath, '   ');
    expect(await readSessionState(statePath)).toEqual({ response_id: '', usage: { inputTokens: 0, cachedTokens: 0, outputTokens: 0, turns: 0 } });
