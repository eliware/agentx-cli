...
export async function createStreamedResponse(openai, request, { liveStreaming = false, statusController = null, debug = false, colors = true, noReasoning = false, noShellCalls = false, noToolCalls = false, noMcp = false, noWebsearch = false } = {}) {
  if (liveStreaming) statusController?.showReasoning();
  const live = createLiveResponseHandlers({ liveStreaming, statusController, colors, noReasoning, noShellCalls, noToolCalls, noMcp, noWebsearch, ...(debug ? { debug: true } : {}) });
