...
export function printAgentText(text) { const wrapped = wrapText(text, getTerminalWidth()); writeTerminal(formatWhiteMessage(wrapped.endsWith('\n') ? wrapped : `${wrapped}\n`)); }
export function printResumeMessage(label, text) { if (!text) return; writeTerminal(`${formatSystemMessage(`${label}:`)}\n`); printAgentText(text); }
...
export function printUsageReport(totals, { leadingNewline = false, model } = {}) { writeTerminal(`${leadingNewline ? '\n' : ''}${formatFinalUsageMessage(formatUsageReport({ ...totals, model }))}\n`); }
