...
import { cleanupStaleOneShotStates, createPendingResponse, getToolCallId, persistCheckpoint, readLatestCheckpoint } from './checkpoint.mjs';
import { createReplInterface, printAgentText, printResumeMessage, printUsageReport } from './repl.mjs';
...
export async function runAgent({ promptPath, cwd, input: terminalInput = defaultInput, output: terminalOutput = defaultOutput, initialMessage = null, oneShot = false } = {}) {
  ...
  if (activeGoal?.status === 'paused' && !oneShot) {
    writeTerminal(`${formatSystemMessage(`Paused goal: ${activeGoal.text}. Use /goal resume to continue.`)}\n`);
  }
  ...
  if (hasPendingToolCalls && !oneShot) {
    ...
    if (resumeChoice === 'new-session' && hasPendingTransaction) {
      ...
      writeTerminal(`${formatSystemMessage('Interrupted work abandoned; returned to the last successful checkpoint.')}\n`);
      ...
      try {
        const resumedResponse = await handleToolCalls(
          ...
          runPendingToolCall,
          {
            ...
            yolo: yoloEnabled,
            onWorkerUsage: (usage) => { addUsageTotals(sessionUsage, usage); sessionUsage.turns += usage.turns || 0; }, onWorkerComplete,
          },
        );
        ...
        if (error?.code === 'previous_response_not_found') {
          ...
          await saveState();
          writeTerminal(`${formatSystemMessage(`Pending response failed: ${error?.message || String(error)}. Session preserved.`)}\n`);
        }
      }
    }
  }
    ...
    for (; ;) {
      ...
      try {
        ...
        if (error?.name === 'AbortError' || error?.code === 'ABORT_ERR') {
          if (activeGoal?.status === 'active') { activeGoal = { ...activeGoal, status: 'cancelled', cancelled_at: new Date().toISOString() }; await saveState(); writeTerminal(`${formatSystemMessage('Goal cancelled')}\n`); continue; }
      ...
      if (message.startsWith('!')) {
        ...
        try {
          ...
          if (interruptedShell) writeTerminal(`${formatSystemMessage('User interrupted command (Ctrl-C)')}\n`);
        } finally {
          if (interactiveShell) {
      ...
      if (internal?.type === 'goal_help') {
        writeTerminal(`${formatSystemMessage('Usage: /goal <text> | /goal status | /goal resume | /goal cancel')}\n`);
        continue;
      }
      ...
      if (internal?.type === 'goal_cancel') {
        if (activeGoal) { activeGoal = { ...activeGoal, status: 'cancelled' }; await saveState(); writeTerminal(`${formatSystemMessage('Goal cancelled')}\n`); }
        else writeTerminal(`${formatSystemMessage('No active goal.')}\n`);
        continue;
      }
      if (internal?.type === 'goal_resume') {
        if (activeGoal?.status !== 'paused') { writeTerminal(`${formatSystemMessage('No paused goal to resume.')}\n`); continue; }
      ...
      if (internal?.type === 'goal') {
        if (activeGoal?.status === 'active') { writeTerminal(`${formatSystemMessage('A goal is already active; cancel it first.')}\n`); continue; }
        if (activeGoal?.status === 'paused') { writeTerminal(`${formatSystemMessage('A paused goal exists; use /goal resume or /goal cancel first.')}\n`); continue; }
      ...
      if (internal?.type === 'rollback') {
        ...
        try {
          ...
          if (selected) {
            ...
            writeTerminal(`${formatSystemMessage('No successful rollback checkpoints available.') }\n`);
          }
        } catch (error) {
          if (error?.name !== 'AbortError') writeTerminal(`${formatSystemMessage(error?.message || String(error))}\n`);
        }
        rl = createReplInterface(() => cwd, terminalInput, terminalOutput, replHistory);
        continue;
      }
      ...
      while (!response) {
        ...
        try {
          ...
          const onViewImage = async ({ args, response: current, previousResponseId, baseRequest, cwd: imageCwd }) => inspectImage(openai, args, {
            cwd: imageCwd,
            ...
            processWorker: true,
            onUsage: (usage) => { addUsageTotals(sessionUsage, usage); sessionUsage.turns += usage.turns || 1; },
          });
          const onWorkerUsage = (usage) => { addUsageTotals(sessionUsage, usage); sessionUsage.turns += usage.turns || 0; };
                    const onGoalIteration = async (iterations) => { if (activeGoal?.status === 'active') { activeGoal = { ...activeGoal, iterations }; await saveState(); } };
          const onGoalComplete = async (result) => {
            activeGoal = { ...activeGoal, status: 'completed', result, completed_at: new Date().toISOString() };
            await saveState();
          ...
          const onGoalLimit = async (iterations) => {
            ...
            await saveState();
            writeTerminal(`${formatSystemMessage(`Goal stopped after ${iterations} iterations`)}\n`);
          };
          try {
            ...
            retryRequest || activeOverride,
            {
              ...
              onGoalComplete,
              onGoalFinalResponse: async (text) => { if (text) printAgentText(text); },
              onGoalBlocked,
              onGoalLimit,
            },
            );
          ...
          if (error?.code === 'previous_response_not_found' && previousResponseId && recoveryAttempts < 1) {
            ...
            retryRequest = null;
            pendingRetryRequest = null;
            writeTerminal(`${formatSystemMessage('Previous response not found; starting a new chain.')}\n`);
            continue;
          }
          failedResponse = true;
          if (!pendingTransaction?.request) pendingToolCalls = [];
