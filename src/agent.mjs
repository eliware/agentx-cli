      ...
      try {
        const resumedResponse = await handleToolCalls(
          ...
          runPendingToolCall,
          {
            ...
            onViewImage: async ({ args, response: current, baseRequest, cwd: imageCwd }) => inspectImage(openai, args, { cwd: imageCwd, responseId: current?.id, model: baseRequest?.model }),
            yolo: yoloEnabled,
          },
        );
