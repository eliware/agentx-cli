      ...
      try {
        const resumedResponse = await handleToolCalls(
          ...
          runPendingToolCall,
          {
            ...
            onViewImage: async ({ args, response: current, previousResponseId, baseRequest, cwd: imageCwd }) => inspectImage(openai, args, { cwd: imageCwd, responseId: current?.id, previousResponseId, callerResponse: current, model: baseRequest?.model }),
            yolo: yoloEnabled,
